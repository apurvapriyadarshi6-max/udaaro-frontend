/** * =============================================================================
 * UDAARO COMMAND BRIDGE - INTEGRATED LOGIC
 * =============================================================================
 */
...
import { 
  Menu, X, ChevronRight, Terminal, Search, Command, ShieldCheck, 
  IndianRupee, Globe2, ArrowUpRight, BookOpen, Activity 
} from "lucide-react";
...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 w-full z-[1000] transition-all duration-500 ${
      isScrolled ? "bg-white/80 backdrop-blur-2xl py-4 border-b border-[#D4AF37]/10 shadow-xl" : "bg-transparent py-10"
    }`}>
      {/* Sovereign Progress Indicator */}
      <motion.div 
        style={{ scaleX: progressLine }} 
        className="absolute top-0 left-0 right-0 h-[3px] bg-[#D4AF37] origin-left z-50 shadow-[0_0_10px_#D4AF37]" 
      />
      ...
      <div className="max-w-[1800px] mx-auto px-10 flex justify-between items-center">
        ...
        <div className="flex items-center gap-12">
          <Link to="/" className="flex items-center gap-4 group">
            <div className="w-12 h-12 bg-[#0F1419] text-[#D4AF37] rounded-xl flex items-center justify-center font-black italic text-xl border border-[#D4AF37]/20 group-hover:rotate-90 transition-transform">U</div>
            <div className="flex flex-col">
              <span className="text-xl font-black text-[#0F1419] uppercase tracking-tighter leading-none italic">Udaaro</span>
              <span className="text-[8px] font-black text-[#D4AF37] uppercase tracking-[0.4em] italic mt-1">Sovereign_OS</span>
            </div>
          </Link>
            ...
            >
              ...
              <AnimatePresence>
                {activeHover === i && (
                  ...
                  >
                    <div className="bg-[#0F1419] rounded-[2.5rem] p-8 border border-[#D4AF37]/20 shadow-2xl">
        ...
        <div className="hidden md:flex items-center gap-6">
          <Link to="/admin-login" className="w-12 h-12 flex items-center justify-center bg-slate-50 border border-slate-200 rounded-xl text-slate-400 hover:text-[#0F1419] hover:bg-white transition-all shadow-sm">
            <Terminal size={18} />
          </Link>
          <Link to="/apply" className="px-10 py-4 bg-[#0F1419] text-white rounded-xl text-[10px] font-black uppercase tracking-[0.4em] italic hover:bg-[#D4AF37] transition-all shadow-lg">
            Join_Batch_2026
          </Link>
        </div>

        {/* MOBILE MENU TRIGGER */}
        <button onClick={() => setIsOpen(!isOpen)} className="md:hidden p-4 bg-white border border-slate-200 rounded-xl">
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div 
            ...
            className="fixed inset-0 bg-[#FDF9F3] z-[900] md:hidden pt-32 px-10 flex flex-col gap-6"
          >
            {NAV_CLUSTERS.map((node) => (
              <Link key={node.path} to={node.path} className="p-8 bg-white border border-slate-100 rounded-[2rem] flex justify-between items-center">
                <span className="text-xl font-black uppercase italic">{node.name}</span>
                <ChevronRight size={20} className="text-[#D4AF37]" />
              </Link>
            ))}
            <Link to="/apply" className="w-full py-8 bg-[#0F1419] text-white rounded-[2rem] text-center font-black uppercase tracking-[0.5em] italic mt-10">
              Initiate_Admission
            </Link>
          </motion.div>
        )}
