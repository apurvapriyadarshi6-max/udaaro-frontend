/**
 * =============================================================================
 * UDAARO SOVEREIGN VENTURE OS - MASTER CORE v6.1.7 (PATCHED_PRODUCTION)
 * -----------------------------------------------------------------------------
 * ARCHITECT: Apurva Priyadarshi
 * FIX: Enforced Wildcard Path Matching for Descendant Route Optimization
 * =============================================================================
 */
...
const SovereignAI = () => {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[10000]">
      <AnimatePresence>
        {state.isOpen && (
          ...
          >
            ...
            <div className="flex-1 p-6 overflow-y-auto space-y-6 bg-[#FDF9F3]/40 scroll-smooth" ref={scrollRef}>
            ...
            <form onSubmit={handleLogicQuery} className="p-6 border-t bg-white flex gap-3 items-center">
              <input value={input} onChange={(e) => setInput(e.target.value)} placeholder="Query logic matrix..." className="flex-1 bg-slate-50 border border-slate-100 rounded-2xl px-5 py-4 text-xs outline-none focus:border-[#D4AF37]/50 transition-all font-medium italic" />
      ...
      <button type="button" onClick={() => dispatch({ type: 'TOGGLE' })} className={`w-16 h-16 md:w-20 md:h-20 rounded-full flex items-center justify-center shadow-2xl transition-all duration-500 border-2 ${state.isOpen ? 'bg-white border-slate-200 text-[#0F1419]' : 'bg-[#0F1419] border-[#D4AF37] text-[#D4AF37]'}`}>
...
// --- MODULE II: LAYOUT INTERIOR MATRIX ---
const UdaaroLayoutMatrix = () => {
  const location = useLocation();
  ...
  return (
    <div className="bg-[#FDF9F3] min-h-screen relative">
      <AnimatePresence>
        {!isResonating && (
          <motion.div initial={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.2 }} className="z-[99999] fixed inset-0">
            <SovereignLoader />
          </motion.div>
        )}
      </AnimatePresence>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <Routes location={location}>
                {/* CRITICAL ATOM UPDATE: Changed path to "/*" 
                   This grants the descendant routes inside Home.jsx the absolute authority 
                   to securely resolve deep tracking sub-paths like /intelligence.
                */}
